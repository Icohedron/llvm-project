...
//===----------------------------------------------------------------------===//
// Compat overload emission
//===----------------------------------------------------------------------===//

/// Emit all overloads for a single HLSLCompatOverload record.
static void emitCompatOverload(raw_ostream &OS, const Record *R) {
  StringRef Name = R->getValueAsString("Name");
  int Arity = R->getValueAsInt("Arity");
  StringRef CastElem =
      R->getValueAsDef("CastType")->getValueAsString("Name");
  StringRef RetElem =
      R->getValueAsDef("ReturnType")->getValueAsString("Name");
  bool EmitScalar = R->getValueAsBit("VaryingScalar");
  std::vector<int64_t> VecSizes = R->getValueAsListOfInts("VaryingVecSizes");
  std::vector<const Record *> Groups = R->getValueAsListOfDefs("Groups");

  OS << "// " << Name << " compat overloads\n";

  for (const Record *Group : Groups) {
    const Record *Deprec = Group->getValueAsDef("Deprecation");
    StringRef DeprecMsg = Deprec->getValueAsString("Message");
    std::vector<const Record *> SrcTypes =
        Group->getValueAsListOfDefs("Types");

    // Build the deprecation attribute string with the function name
    // substituted for {0}.
    std::string DeprecAttr;
    if (!DeprecMsg.empty()) {
      std::string Msg = DeprecMsg.str();
      size_t Pos = Msg.find("{0}");
      if (Pos != std::string::npos)
        Msg.replace(Pos, 3, Name.str());
      DeprecAttr =
          "[[deprecated(\"" + Msg + "\")]]\n";
    }

    for (const Record *SrcType : SrcTypes) {
      StringRef SrcElem = SrcType->getValueAsString("Name");

      auto EmitOne = [&](StringRef SrcTy, StringRef CastTy, StringRef RetTy) {
        OS << DeprecAttr;
        OS << "constexpr " << RetTy << " " << Name << "(";
      ...
      if (EmitScalar)
        EmitOne(SrcElem, CastElem, RetElem);
      for (int64_t N : VecSizes)
        EmitOne(getVectorTypeName(SrcElem, N), getVectorTypeName(CastElem, N),
                getVectorTypeName(RetElem, N));
    }
  }
  OS << "\n";
}

void clang::EmitHLSLCompatIntrinsics(const RecordKeeper &Records,
                                     raw_ostream &OS) {
  OS << "// This file is auto-generated by clang-tblgen from "
        "HLSLIntrinsics.td.\n";
  OS << "// Do not edit this file directly.\n\n";
  ...
  for (const Record *R :
       Records.getAllDerivedDefinitions("HLSLCompatOverload"))
    emitCompatOverload(OS, R);
}
